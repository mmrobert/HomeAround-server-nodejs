...
router.use(function(req, res, next) {
   ...
   //    console.log('second 3x3x3x!' + 'HaHaha');
         uHasGeo = false;
       }
     } else {
        uHasGeo = false;
...
router.post('/', function(req, res, next) {
  ...
  Customer.findOneAndUpdate({email: req.uemail}, 
      {$set:{name: nameHH, photo: photoHH, postcode: postcodeHH, country:countryHH, loc: req.uloc}}, 
      function(err, user) {
           if (err) {
              res.json({
                   success: false,
                   message: 'Error in updating customer profile!'});
           } else {
              res.json({
                  success: true,
