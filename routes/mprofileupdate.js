...
router.post('/', function(req, res, next) {
  var nameHH = req.body.name;
  var photoHH = req.body.photo;
  var postcodeHH = req.body.postcode;
  var countryHH = req.body.country;
  var phoneHH = req.body.phone;
  var expertiseHH = req.body.expertise;
  var biodetailHH = req.body.biodetail;
  //   console.log('second 303030!' + emailH);
  Merchant.findOneAndUpdate({email: req.uemail}, 
        {$set:{name: nameHH, photo: photoHH, postcode: postcodeHH, phone: phoneHH, 
        biodetail: biodetailHH, country:countryHH, loc: req.uloc, expertise: expertiseHH}}, 
         function(err, user) {
           if (err) {
              res.json({
                   success: false,
