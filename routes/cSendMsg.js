...
router.use(function(req, res, next) {
 ...
 //  User.findOne({email: 'keep@keep.ca'}, function (err, user) {
    if(user) {
       req.merDevice = user.deviceToken;
       req.merActive = user.deviceActive;
       req.merPlatform = user.platform;
       next();
    } else {
  //     console.log('No User 909090!' + req.body.email);
...
router.post('/', function(req, res, next) {
  ...
  //     console.log('No User 909090!' + req.body.email);
         if(req.merPlatform == 'ios' && req.merActive) {
           var apnDevice = new apn.Device(req.merDevice);
         ...
         } else if(req.merPlatform == 'android' && req.merActive) {
           var msgsSendingH = {
              to: req.merDevice,
